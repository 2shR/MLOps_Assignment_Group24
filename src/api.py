...
def retrain():
    ...
    if result.returncode == 0:
        result_register = subprocess.run(["python", "src/register_model.py"], capture_output=True, text=True)
        if result_register.returncode == 0:
            return jsonify({"status": "success", "output": result_register.stdout})
        else:
            return jsonify({"status": "error", "error": result_register.stderr}), 500
    else:
